/*************************************************************************
	...
	> Author: 
	> Mail: 
	> Created Time: 2017年05月25日 星期四 12时29分14秒
 ************************************************************************/
...
public:
    ...
    std::string Encode(std::ifstream& infile);
...
private:
    ...
    std::string ByteToHexString(const Byte* input, size_t length);
...
void CMessageDigestAlgorithm5::ProcessOfMDA5(const Byte  groups[64]){
     ...
     *    *返回值：16进制字符串
     *    ****************************************************/
std::string CMessageDigestAlgorithm5::ByteToHexString(const Byte* input, size_t length){
     ...
     * *返回值：MD5加密后算列值
     * ****************************************************/
std::string CMessageDigestAlgorithm5::Encode(std::string &str){
    Initialize();
    EncodeByte((const Byte * )(str.c_str()), str.length());
    Final();
    std::string strMD5 = ByteToHexString(m_Result,16);
    return strMD5;
}
    /***************************************************
    * *参数：infile表示待加密文件
    ...
    * *返回值：MD5加密后算列值
    * ****************************************************/
std::string CMessageDigestAlgorithm5::Encode(std::ifstream & infile) {
    if (!infile){
        return "";
    }
    Initialize();
